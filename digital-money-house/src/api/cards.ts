...
export async function getAllCardsByAccount( accountID: number ) {
    try {
        const response = await digitalMoneyApi.get("/api/accounts/" + accountID + "/cards"  );
...
export async function createCard( accountID: number, newCard: {}) {
    try {
        const response = await digitalMoneyApi.post("/api/accounts/" + accountID + "/cards", newCard );
        return response.data;
    } catch (error) {
        console.error("Failed to do it!", error);
...
export async function getCardByID( accountID: number, cardID: number ) {
    try {
        const response = await digitalMoneyApi.get("/api/accounts/" + accountID + "/cards" + cardID);
        return response.data;
    } catch (error) {
        console.error("Failed to do it!", error);
